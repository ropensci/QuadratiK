...
#' @importFrom ggplot2 ggplot geom_line geom_point geom_hline labs theme_minimal theme_light theme scale_y_continuous
...
select_h <- function(x,
   ...
   objective_2 <- function(h, k) {
      ...
      CV <- compute_CV(B, Quantile, pooled, n, m, h, method, b, compute_variance = FALSE)
      cv <- CV$cv
   ...
   objective_k <- function(h, k) {
      ...
      STATISTIC <- stat_ksample_cpp(xnew, ynew, h, sizes_new, cum_size_new, compute_variance = FALSE)
      CV <- cv_ksample(xnew, ynew, h, B, b, Quantile, method, compute_variance = FALSE)
      cv <- CV$cv
      return(c(STATISTIC[1] < cv[1]))
   }
   ...
   for (k in k_values) {
      results <- foreach::foreach(
         i = seq_along(params),
         .combine = rbind,
         .packages = c("sn", "moments", "stats", "rlecuyer")
      ) %dopar% {
      ...
      if (nrow(min_h_power_gt_05) > 0) {
         ...
         selected_point <- min_h_power_gt_05[1, ]
         break
      } else {
         min_h <- NULL
      }
   ...
   pl <- ggplot(res, aes(x = h, y = power)) +
      geom_line(aes(col = as.factor(delta)),
                linewidth = 0.9,
                alpha = .9) +
      geom_point(aes(col = as.factor(delta)), size = 2) +
      geom_hline(yintercept = 0.5, linetype = "dashed") +
      geom_point(data = selected_point,
                 aes(x = h, y = power),
                 shape = 13,
                 size = 5) +
      labs(y = "Power") +
      ...
      scale_y_continuous(breaks = function(x) sort(unique(c(pretty(x), 0.5))))
   
   results <- list(h_sel = min_h,
                   power = res,
                   power.plot = pl)
